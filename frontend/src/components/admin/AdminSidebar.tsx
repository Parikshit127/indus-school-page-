...
interface AdminSidebarProps {
    activeTab: 'dashboard' | 'content' | 'gallery' | 'members' | 'calendar' | 'teachers' | 'analytics' | 'news' | 'results' | 'popups' | 'achievements';
    setActiveTab: (tab: 'dashboard' | 'content' | 'gallery' | 'members' | 'calendar' | 'teachers' | 'analytics' | 'news' | 'results' | 'popups' | 'achievements') => void;
    onLogout: () => void;
    isOpen?: boolean;
    onClose?: () => void;
}
        ...
        <div className={`w-64 bg-royal-dark min-h-screen text-white flex flex-col fixed left-0 top-0 h-full z-50 shadow-xl transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'
            ...
            <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                ...
                <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${activeTab === 'achievements'
                        ? 'bg-gold text-royal-dark font-bold shadow-lg shadow-gold/20'
                        : 'text-white/70 hover:bg-white/5 hover:text-white'
                        }`}
                >

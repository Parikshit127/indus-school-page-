...
import { Save, Upload, FileText, Loader2, Download, Trash2, Receipt } from "lucide-react";
import { motion } from "framer-motion";

export function FeesManager() {
    const [pdfUrl, setPdfUrl] = useState("");
    const [title, setTitle] = useState("Fees Structure"); // Default title
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/api/content/fees/upload`, {
                method: 'POST',
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (response.ok) {
                setMessage({ type: 'success', text: 'Fees structure updated successfully!' });
            } else {
    ...
    return (
        <div className="max-w-4xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h2 className="text-xl md:text-2xl font-serif font-bold text-royal">Fees Structure Manager</h2>
                    <p className="text-sm md:text-base text-slate-500">Manage the fees structure PDF and title</p>
                </div>
                <button
                    onClick={handleSave}
                    disabled={saving}
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-6">
                <div>
                     <label className="block text-sm font-bold text-royal mb-2">Fees Structure Title</label>
                     <input 
                        type="text" 
                        value={title} 
                        ...
                        placeholder="e.g. Fees Structure 2024-25"
                     />
                </div>
                        ...
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200 gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="overflow-hidden">
                                    <p className="font-medium text-slate-700">Current Fees PDF</p>
                                    <a 
                                        href={pdfUrl} 
                                        target="_blank" 
